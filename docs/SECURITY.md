...
| `root` 密钥带来特权访问     | 仅对用户明确配置的 `root` 账号写入 `/root/.ssh`；不使用 `sudo`；保留同样的主机信任、所有权、权限、加锁和精确撤销约束 |
...
- UID `0` 是有效的 Linux 账号身份，不能仅因为是 `root` 而绕过或拒绝布局校验；它只接受精确的标准 `/root`，并与普通账号使用相同的精确 UID、文件类型、禁止组/其他用户写入、并发和原子替换检查。
- 参照 OpenSSH `StrictModes` 的安全边界，拒绝组/其他用户可写的 `.ssh` 或 `authorized_keys`；只读/只执行权限不会仅因为不是推荐的 `700`/`600` 就被误判。
- 拒绝符号链接、异常所有者和非普通文件。
