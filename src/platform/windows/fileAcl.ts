...
export class WindowsFileAcl {
  ...
  private async restrict(
    ...
    const script = [
      ...
      'function Read-OwnerSid($acl) { try { $value = $acl.GetOwner([System.Security.Principal.SecurityIdentifier]) } catch { exit 46 }; if ($null -ne $value -and [string]::IsNullOrWhiteSpace($value.Value)) { exit 46 }; return $value }',
