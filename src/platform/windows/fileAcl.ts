...
export class WindowsFileAcl {
  ...
  private async restrict(
    ...
    const script = [
      ...
      `Add-Type -TypeDefinition 'using System; using System.Runtime.InteropServices; public static class SshOnboardNativeAcl { [DllImport("advapi32.dll", CharSet = CharSet.Unicode)] public static extern uint GetNamedSecurityInfo(string objectName, int objectType, uint securityInfo, out IntPtr owner, IntPtr group, IntPtr dacl, IntPtr sacl, out IntPtr securityDescriptor); [DllImport("kernel32.dll")] public static extern IntPtr LocalFree(IntPtr memory); }' -ErrorAction Stop`,
      'function Read-OwnerSid($acl, [string]$target, [int]$failureBase) { if ($acl -isnot [System.Security.AccessControl.FileSystemSecurity]) { exit $failureBase }; $ownerPointer = [IntPtr]::Zero; $descriptorPointer = [IntPtr]::Zero; $nativeFailed = $false; $freeFailed = $false; $ownerValue = $null; try { try { $status = [SshOnboardNativeAcl]::GetNamedSecurityInfo($target, 1, 1, [ref]$ownerPointer, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [ref]$descriptorPointer) } catch { $nativeFailed = $true }; if (-not $nativeFailed -and ($status -ne 0 -or $descriptorPointer -eq [IntPtr]::Zero)) { $nativeFailed = $true }; if (-not $nativeFailed -and $ownerPointer -ne [IntPtr]::Zero) { try { $ownerValue = ([System.Security.Principal.SecurityIdentifier]::new($ownerPointer)).Value } catch { $nativeFailed = $true } } } finally { if ($descriptorPointer -ne [IntPtr]::Zero) { $freeFailed = [SshOnboardNativeAcl]::LocalFree($descriptorPointer) -ne [IntPtr]::Zero } }; if ($nativeFailed) { exit $failureBase }; if ($freeFailed) { exit ($failureBase + 1) }; if ($ownerPointer -eq [IntPtr]::Zero) { return $null }; if ([string]::IsNullOrWhiteSpace($ownerValue)) { exit ($failureBase + 2) }; return $ownerValue }',
      "$isDirectory = $mode -eq 'directory' -or $mode -eq 'check-directory'",
      "$checkOnly = $mode.StartsWith('check-')",
      '$existing = if ($isDirectory) { [System.IO.Directory]::GetAccessControl($target) } else { [System.IO.File]::GetAccessControl($target) }',
      '$owner = Read-OwnerSid $existing $target 50',
      '$ownerIsCurrent = $null -ne $owner -and $owner -eq $current.Value',
      '$ownerIsTrusted = $null -ne $owner -and ($ownerIsCurrent -or $owner -eq $administrators.Value -or $owner -eq $system.Value)',
      ...
      '$ownerAcl = if ($isDirectory) { [System.IO.Directory]::GetAccessControl($target) } else { [System.IO.File]::GetAccessControl($target) }',
      '$intermediateOwner = Read-OwnerSid $ownerAcl $target 60',
      "$icacls = Join-Path $env:SystemRoot 'System32\\icacls.exe'",
      "if ($null -eq $intermediateOwner -or ($intermediateOwner -ne $current.Value -and $intermediateOwner -ne $administrators.Value -and $intermediateOwner -ne $system.Value)) { try { $ownerAcl.SetOwner($current); if ($isDirectory) { [System.IO.Directory]::SetAccessControl($target, $ownerAcl) } else { [System.IO.File]::SetAccessControl($target, $ownerAcl) } } catch { & $icacls $target /setowner ('*' + $current.Value) /L /Q | Out-Null; if ($LASTEXITCODE -ne 0) { exit 45 } } }",
      '$actual = if ($isDirectory) { [System.IO.Directory]::GetAccessControl($target) } else { [System.IO.File]::GetAccessControl($target) }',
      '$actualOwner = Read-OwnerSid $actual $target 70',
      ...
      'if ($null -ne $actualOwner -and $actualOwner -ne $current.Value -and $actualOwner -ne $administrators.Value -and $actualOwner -ne $system.Value) { exit 44 }',
    ].join('; ');
