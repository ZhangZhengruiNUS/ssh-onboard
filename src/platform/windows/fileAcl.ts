...
export class WindowsFileAcl {
  ...
  private async restrict(
    ...
    const script = [
      ...
      'if ($intermediateOwner.Value -ne $current.Value) { $ownerAcl.SetOwner($current); if ($isDirectory) { [System.IO.Directory]::SetAccessControl($target, $ownerAcl) } else { [System.IO.File]::SetAccessControl($target, $ownerAcl) } }',
