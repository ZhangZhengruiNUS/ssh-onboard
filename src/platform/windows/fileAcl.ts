...
export class WindowsFileAcl {
  ...
  private async restrict(
    ...
    const script = [
      ...
      "function Read-OwnerSid($acl, [int]$failureBase) { if ($acl -isnot [System.Security.AccessControl.FileSystemSecurity]) { exit $failureBase }; $previousPreference = $ErrorActionPreference; try { $ErrorActionPreference = 'Stop'; $ownerValue = $acl.Owner; $ownerReadSucceeded = $? } catch { exit $failureBase } finally { $ErrorActionPreference = $previousPreference }; if (-not $ownerReadSucceeded) { exit $failureBase }; if ($null -eq $ownerValue) { return $null }; if ($ownerValue -isnot [string]) { exit ($failureBase + 1) }; if ($ownerValue.Length -eq 0) { return $null }; if ([string]::IsNullOrWhiteSpace($ownerValue)) { exit ($failureBase + 2) }; try { $value = ([System.Security.Principal.NTAccount]$ownerValue).Translate([System.Security.Principal.SecurityIdentifier]) } catch { exit ($failureBase + 3) }; if ($null -eq $value -or [string]::IsNullOrWhiteSpace($value.Value)) { exit ($failureBase + 3) }; return $value }",
