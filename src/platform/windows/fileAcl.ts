...
export class WindowsFileAcl {
  ...
  private async restrict(
    ...
    const script = [
      ...
      '$ownerIsTrusted = $ownerIsCurrent -or $owner.Value -eq $administrators.Value',
      ...
      "if ($intermediateOwner.Value -ne $current.Value -and $intermediateOwner.Value -ne $administrators.Value) { & $icacls $target /setowner ('*' + $current.Value) /L /Q | Out-Null; if ($LASTEXITCODE -ne 0) { exit 44 } }",
      ...
      'if ($actualOwner.Value -ne $current.Value -and $actualOwner.Value -ne $administrators.Value) { exit 44 }',
