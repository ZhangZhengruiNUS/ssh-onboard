...
export class WindowsFileAcl {
  ...
  private async restrict(
    ...
    const script = [
      ...
      'function Get-OwnerSid($acl) { try { $value = $acl.GetOwner([System.Security.Principal.SecurityIdentifier]); if ($null -eq $value -or [string]::IsNullOrWhiteSpace($value.Value)) { return $null }; return $value } catch { return $null } }',
      ...
      'if ($checkOnly -and -not $ownerIsTrusted) { exit 43 }',
      'if (-not $ownerIsTrusted -and -not $createdByUs) { exit 43 }',
      ...
      'if ($ownerIsTrusted -and $existingExact) { exit 0 }',
      '$acl = $existing',
      ...
      '$intermediateOwner = Get-OwnerSid $ownerAcl',
      ...
      '$actualOwner = Get-OwnerSid $actual',
      'if ($null -eq $actualOwner -or ($actualOwner.Value -ne $current.Value -and $actualOwner.Value -ne $administrators.Value -and $actualOwner.Value -ne $system.Value)) { exit 44 }',
