...
- 账号：主目录可写的 Linux 用户；在确实需要特权密钥登录时也支持 `root`。

`root` 初始化会安装一把可免密获得特权访问的密钥，可能时仍应优先使用普通账号。跳板机、MFA、非标准 `AuthorizedKeysFile`、加密的已有私钥，以及 macOS/Linux 本地客户端暂不属于 V0.1 的已测试范围。
